    ...
    var functions = {
        ...
        bmi: function (obj) {
            if (obj.hasOwnProperty('height') && obj.hasOwnProperty('weight')) {
                ...
                if (height > 0 && weight > 0) return weight / ((height / 100) * (height / 100));
            }
            return -1;
        },
    };
    var checkInBuiltFn = function (keyToCheck) {
        if (functions.hasOwnProperty(keyToCheck)) return true;
        return false;
    };
    var args = {};
    ...
    var parseNode = function (node, anyKLevel, oldResults) {
        ...
        if (operators[node.type]) {
            ...
            if (node.right) {
                ...
                if (node.type == '||') {
                    if (leftval) levelToKMap[anyKLevel] = levelToKMap[anyKLevel] - 1;
                    if (rightval) levelToKMap[anyKLevel] = levelToKMap[anyKLevel] - 1;
                    if (levelToKMap[anyKLevel] <= 0) return true;
                    else return false;
                }
                return outval;
            }
            ...
            for (var i = 0; i < node.args.length; i++)
                node.args[i] = parseNode(node.args[i], anyKLevel, oldResults);
