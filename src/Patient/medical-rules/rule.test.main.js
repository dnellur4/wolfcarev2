...
function attachEventHandlesForTesting() {
    ...
    $('.testtab').on('click', function () {
        ...
        if (newSavedRules.length > 0) {
            // newSavedRules and savedRulesJSON are declared/defined in rule.addedit.main.js
    ...
    for (var key in result) {
        ...
        if (typeof result[key] == 'object') {
            ...
            for (var inkey in result[key]) {
                ...
                if (result[key][inkey] === true) foundclass = "style='color:blue'";
                htmlStr += '<li ' + foundclass + '>' + inkey + ':' + result[key][inkey] + '</li>';
            }
            htmlStr += '</ul>';
        } else {
            var foundclass = "style='color:black'";
            if (result[key] === true) foundclass = "style='color:blue'";
        ...
        for (key in ruleObj) {
            $('.testrulesdiv').append(
                "<br/><input type='checkbox' value = '" +
                    key +
                    "' class='checkrule' checked='checked'/>" +
                    key
            );
            break;
        }
...
function pushTokens(tokens, labAttributes, labFormMap) {
    for (var k = 0; k < tokens.length; k++) {
        if (labValueMap.hasOwnProperty(tokens[k]) && !labFormMap.hasOwnProperty(tokens[k])) {
            ...
            } else labAttributes.push({ id: tokens[k], name: labValueMap[tokens[k]] });
        }
    }
}
...
function populateLabForm(selectedRules) {
    ...
    for (var i = 0; i < labAttributes.length; i++) {
        ...
        var val = 0;
        $('.labvalueformdiv').append(
            "<div class='labformrow' style='height:30px'><span style='width:300px;display:inline-block'>" +
                name +
                ":</span><input type='text' id='" +
                id +
                "' style='width:100px'/><span class='nameerror'></span></div>"
        );
    }
}
