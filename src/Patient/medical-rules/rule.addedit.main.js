...
function attachEventHandlers() {
    ...
    $('#displayassoc').on('change', function () {
        ...
        } else config.displayAssociations = false;
    });
    ...
    $('.addtab').click(function () {
        ...
        if (preselectedOptionsInAddArray != null) {
            ...
            for (var i = 0; i < preselectedOptionsInAddArray.length; i++) {
                preselectOptions.push({
                    id: preselectedOptionsInAddArray[i],
                    name: labValueMap[preselectedOptionsInAddArray[i]],
                });
            }
        }
        organizeOptions(preselectOptions, '#addrule');
    ...
    $('.edittab').click(function () {
        ...
        if (newSavedRules.length > 0) {
            for (var i = 0; i < newSavedRules.length; i++) savedRulesJSON.push(newSavedRules[i]);
            newSavedRules = [];
        }
        populateDefinedRules(savedRulesJSON);
    });
    ...
    $('#addrule .preselect h2').click(function (e) {
        ...
        else $(this).children('.expandcollapse').text('+');
    });
    ...
    $('#editrule .multipreselect').on('select2:select select2:unselect', function (e) {
        ...
        $.each(items, function (index, element) {
            ...
            if (doAdd) preselectOptions.push({ id: element.value, name: element.text });
        });
    ...
    $('.rulename').keyup(function () {
        ...
        if (
            $(obj).children('div.categorycomponent').find('.hassubcategory').is(':checked') ===
            false
        ) {
            $(obj)
                .children('div.ruleconditioncomponent')
                .find('.ruleconditions h2 .definition')
                .val($(this).val())
                .attr('disabled', true);
        }
    });
    ...
    $('.ruleconditioncomponent').on(
        'click',
        '.ruleTemplate .condition, .ruleTemplate .conditionlabvalue',
        function () {
            ...
            if (config.displayAssociations) {
                addAssociations(
                    $(this).siblings('.anyall').val(),
                    $(this).parents('.ruleTemplate')
                ); // this.parent.parent refers to ruleTemplate
            }
            populateRuleText($(this).parents());
        }
    );
    ...
    $('.ruleconditioncomponent').on('click', '.ruleTemplate .conditionsubrule', function () {
        var html = $('#conditionTemplate').html();
        $(this).parents('.groupclass').next().append(html); // append html to 'conditionclass' div
        populatePrevDefinedConditionsInLHS(
            $(this)
                .parents('.groupclass')
                .next()
                .children('div.acondition')
        ...
        $(this)
            ...
            .children('.rhs')
            .remove();
        keepOnlyExistsOrNotExistsOperators(
            $(this)
                .parents('.groupclass')
                .next()
                .children('div.acondition')
    ...
    $('.ruleconditioncomponent').on('change', '.anyall', function () {
        ...
        if (config.displayAnyK) {
            if (anyAll == 'OR') {
                $(
                    "<input type='number' value='1' min='1' class='inputforany'></input>"
                ).insertAfter($(this));
            } else {
                $(this).next().remove();
            }
    ...
    $('.ruleconditioncomponent').on('change', '.acondition .operator', function () {
        ...
        if (selectedId === CONSTANTS.NOTPRESENT || selectedId === CONSTANTS.PRESENT) {
            if ($(this).next().attr('class') != 'removecondition') $(this).next().hide();
        } else {
            $(this).next().show();
        }
        populateRuleText($(this).parents());
    ...
    $('.ruleconditioncomponent').on('change', '.acondition .rhs', function () {
        ...
        if (rhsVal == CONSTANTS.NUMBERSTRING || rhsVal == CONSTANTS.CONSTANTSTRING) {
            if ($(this).next().prop('nodeName') !== 'INPUT') {
                $(
                    "<input type='text' value='0' style='width:50px;margin-left:5px;margin-right:5px'/>"
                ).insertBefore($(this).parent().children().last());
                $(this).next().focus();
                $(this).next().select();
            ...
            if ($(this).next().prop('nodeName') === 'INPUT') $(this).next().remove();
    ...
    $('.ruleconditioncomponent').on('click', '.isrulevalid button.clear', function () {
        ...
        $(this)
            .parents('.textandlogicalform')
            .children('.logicalform')
            .append($('#subcategoryTemplate .logicalform').html());
    ...
    $('#addrule').on('click', '.saveAdd', function () {
        // Save Function on adding a new rule
    ...
    $('#editrule').on('click', '.saveEdit', function () {
        // Save Function
    ...
    if (config.usePreviouslyDefinedorSavedRules) {
        for (var i = 0; i < otherDefinedRules.length; i++) {
            for (key in otherDefinedRules[i]) {
                ...
                $(lhsObj).append(
                    '<option value="' + concatStr + '" class="otherdefined">' + key + '</option>'
                );
                break;
            }
        }
...
var checkForNodeValueAsPrevDefinedCondition = function (
    definedRules,
    nodeValue,
    conditionName,
    conditionCategories
...
function addAssociations(anyAll, ruleTemplateObj) {
    ...
    for (var i = 0; i < cachedChildren.length; i++) {
        ...
        if (prevChild != null) {
            ...
            if (
                (prevClass == 'ruleTemplate' && prevClass == currClass) ||
                (prevClass == 'conditionclass' &&
                    prevConditionLength > 0 &&
                    currClass == 'ruleTemplate')
            ) {
...
function invokeModal(message, kindOfMessage) {
    ...
    if (kindOfMessage == 'SUCCESS') $('div.modalTemplate').addClass('successdiv');
    $('div#modalmessage').html(message);
    $('#tabs-container').css({ opacity: 0.1 });
    $('body').css({ 'background-color': 'brown' });
}

function triggerSuccessfulAddition() {
    //TODO beautify alert
...
function validateFieldsOnSave(parentId) {
    ...
    for (var i = 0; i < ruleConditionsObjArray.length; i++) {
        ...
        if (ruleconditions.length == 0) {
            ...
            $(ruleConditionsObjArray[i])
                .find('.nameerror')
                .html(html + '&nbsp;No conditions defined!');
            errorCount++;
        }
    }
...
function organizeOptions(optionsToPopulate, parentId, areOptionsPrePopulatedForEditView) {
    ...
    $.each(optionsToPopulate, function (key, val) {
        ...
        if (!found) $selectN.append('<option value="' + val.id + '">' + val.name + '</option>');
    });
    $selectN = $('.rhs');
...
function determineCurrentRuleDiv(parents) {
    for (var i = 0; i < parents.length; i++) {
        if (parents[i].className === 'textandlogicalform') return parents[i];
    }
    return null;
}
...
function populateRuleText(parents) {
    ...
    if (parentObj) {
        var interpretedRuleText = getRuleString(
            $(parentObj).children('.logicalform').children('.ruletemplate')
        );
...
var parseTokensToText = function (interpText) {
    ...
    var backtrackAndPlugAnyK = function (textArr) {
        ...
        var insertAnyKText = function (curIndex, kvalue, arr) {
            var strStart =
                "<div><span class='textForAny'>Any " +
                kvalue +
                " of </span><br/><span class='textForAny'>(</span><div style='margin-left:50px'>";
            ...
            if (inIndex > 0) {
                ...
                for (var i = curIndex + 1; i < arr.length; i++) {
                    ...
                    if (arr[i]['id'] === 'groupend') {
                        if (groupStartOccurred) groupStartOccurred = false;
                        else {
                            outIndex = i;
                            break;
                        }
        ...
        for (var i = 0; i < tokens.length; i++) {
            ...
            if (labValueMap.hasOwnProperty(tokens[i])) {
                ...
                if (i > 0 && $.inArray(tokens[i - 1], ['>', '<', '>=', '<=']) >= 0) {
                    ...
                    strObj['id'] = 'lhs';
                    strObj['str'] =
                        "<span style='color:green'>" + labValueMap[tokens[i]] + '</span>';
                    retArr.push(strObj);
                }
            }
            // checking for operators and translating them to the text form
                ...
                if (tokens[i] === '(') {
                    ...
                    if (tokens[i - 1] != ')') {
                        //Just to save an extra line break in the user visible text form of the rule (can be omitted)
                        strObj = {};
                        strObj['id'] = 'andorsupp';
                        strObj['str'] = '<br/>';
                        retArr.push(strObj);
                ...
                if (/_/.test(dispStr)) {
                    // Underscores in token imply that can be one of the previously defined comorbidities or comorbidity condition
...
var parseTokensToHTML = function (
    interpText,
    labValuesMap,
    definedRules,
    currentName,
    ...
    var parent = $(
        '#editrule div.ruleconditions:last div.logicalform div.ruleTemplate:last p:first'
    );
    /* 
      node : Current node
        ...
        if (node.hasOwnProperty('group')) {
            if (node.type === '||' || node.type === '&&') {
                ...
                if ($(tempParent).prop('nodeName') === 'P') {
                    // checking for paragraph which only has individual conditions
            ...
            $.inArray(node.type, ['&&', '||', 'identifier', '*']) == -1 &&
            multipleMode == 0
        ) {
            conditionAppend = true;
        }
        if (identifierOnWhichSide === 'LEFT') {
            if (multipleMode == 1 && node.type == 'number') {
                ...
                if (labValuesMap.hasOwnProperty(node.value)) {
                    ...
                    checkForNodeValueAsPrevDefinedCondition(
                        definedRules,
                        node.value,
                        currentName,
                        currentSubCategories
                    ...
                    populatePrevDefinedConditionsInLHS(
                        $(parent).find('.acondition:last .lhs'),
                        definedRules
                    );
                    $(parent).find('.acondition:last .lhs').val(node.value);
                }
            ...
            if (node.type === CONSTANTS.NUMBERSTRING || node.type === CONSTANTS.CONSTANTSTRING) {
                // special case for numerical input
                ...
                $('.acondition:last .rhs')
                    .clone()
                    .insertAfter($(parent).find('.acondition:last input'));
        ...
        if (addcondition || conditionAppend) {
            ...
            if (
                ...
                if (node.type == '==-1' || node.type == '!=-1') {
                    // special case for present/not present
            ...
            identifierOnWhichSide = '';
            if (node.type == '*') multipleMode = 1;
            else multipleMode = 0;
            recurseNode(
                node.left,
                parent,
                conditionAppend,
                multipleMode == 0 ? identifierOnWhichSide : 'LEFT',
                false,
                multipleMode
            );
            recurseNode(
                ...
                parent,
                conditionAppend,
                multipleMode == 0 ? identifierOnWhichSide : 'RIGHT',
                false,
                multipleMode == 1 ? 2 : 0
            );
        }
    ...
    if (hasSubCategories) {
        ...
        for (var j = 0; j < subCategoriesCount; j++) {
            ...
            for (key in subCategories[j]) {
                ...
                $('#editrule .ruleconditions:last .textrule')
                    .html(ruleStr)
...
function getRuleString(obj) {
    ...
    if (andOr === 'Any') {
        ...
        if (anyKval > 1) andOrIntepreter = ' ||' + anyKval + ' ';
    }
...
function addSubCategory(parentDiv, firstCategory) {
    ...
    $(parentDiv)
        .find('.ruleconditioncomponent .ruleconditions:last .removegroupcondition:first')
        .remove();
    $(parentDiv)
        .find('.ruleconditioncomponent .ruleconditions h2')
        .children('.expandcollapse')
        .text('+');
